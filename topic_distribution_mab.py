...
protocol = sys.argv[2]
MAB = int(sys.argv[3]) # 1 means MAB and 0 means deterministic, 2 means oracle, 3 means smart oracle # all topic # of relevant and non-relevant are know
run_number = int(sys.argv[4]) #5
BLA_param = int(sys.argv[5]) #1
BLA_alpha = BLA_param
BLA_beta = BLA_param
...
budget_list_TREC8 = [10134, 19231, 28125, 36947, 45584, 54071, 62476, 70654, 78805, 86830]

ranker_location = {}
ranker_location["WT2013"] = "/work/04549/mustaf/lonestar/data/TREC/WT2013/input.ICTNET13RSR2"
ranker_location["WT2014"] = "/work/04549/mustaf/lonestar/data/TREC/WT2014/input.Protoss"
ranker_location["gov2"] = "/work/04549/mustaf/lonestar/data/TREC/gov2/input.indri06AdmD"
ranker_location["TREC8"] = "/work/04549/mustaf/lonestar/data/TREC/TREC8/input.ibmg99b"
...
if deterministic == 'True':
    deterministic = True
    #base_address = "/work/04549/mustaf/lonestar/data/TREC/deterministic/"
    lambda_param = 0.0
    alpha_param = 1
else:
    deterministic = False
...
if ht_estimation == 'True':
    ht_estimation = True
    #base_address = "/work/04549/mustaf/lonestar/data/TREC/estimation/"
else:
    ht_estimation = False
...
if MAB == 1:
    base_address = base_address + "MAB/"
elif MAB == 0:
    base_address = base_address + "serial/"
elif MAB == 2: # MAB ==2
    base_address = base_address + "oracle/"
elif MAB == 3: # MAB ==3
    base_address = base_address + "smartoracle/"

base_address = base_address + str(run_number) + "/"
if os.path.isdir(base_address) == False:
    os.mkdir(base_address)

base_address = base_address + "BLAparams_"+str(BLA_param) + "/"
if os.path.isdir(base_address) == False:
    os.mkdir(base_address)
...
print "base address:", base_address
if datasource=='TREC8':
    processed_file_location = '/work/04549/mustaf/lonestar/data/TREC/TREC8/processed.txt'
    RELEVANCE_DATA_DIR = '/work/04549/mustaf/lonestar/data/TREC/TREC8/relevance.txt'
    start_topic = 401
    end_topic = 451
    datasetsize = 86830
elif datasource=='gov2':
    processed_file_location = '/work/04549/mustaf/lonestar/data/TREC/gov2/processed.txt'
    RELEVANCE_DATA_DIR = '/work/04549/mustaf/lonestar/data/TREC/gov2/qrels.tb06.top50.txt'
    start_topic = 801
    end_topic = 851
    datasetsize = 31984
elif datasource=='WT2013':
    processed_file_location = '/work/04549/mustaf/lonestar/data/TREC/WT2013/processed_new.txt'
    RELEVANCE_DATA_DIR = '/work/04549/mustaf/lonestar/data/TREC/WT2013/qrelsadhoc2013.txt'
    start_topic = 201
    end_topic = 251
    #datasetsize = 14474 #contains 13265 docs after skipping the topic in skip list
    datasetsize = 13265
else:
    processed_file_location = '/work/04549/mustaf/lonestar/data/TREC/WT2014/processed_new.txt'
    RELEVANCE_DATA_DIR = '/work/04549/mustaf/lonestar/data/TREC/WT2014/qrelsadhoc2014.txt'
    start_topic = 251
    end_topic = 301
    datasetsize = 13483
    #datasetsize = 14432 # contains 13483 docs after skipping the topic in skip list

# not going upto last point
# for example for WT2014 last buf=dget will be 14000, and in the loop we will run until 13000,
# because at 14000 all are same
...
def get_topic_distribution():
    ...
    topic_non_related_documents = {}
    topic_X = {}
    topic_y = {}
    topic_docIndex_DocNo = {}
    ...
    for topic in xrange(start_topic, end_topic):
        ...
        topic_related_documents[topic] = numberOne
        topic_non_related_documents[topic] = numberZero
        topic_X[topic] = X
        topic_y[topic] = y
        topic_docIndex_DocNo[topic] = docIndex_DocNo

    return topic_initial_X_train, topic_initial_Y_train, topic_seed_one_counter, topic_seed_zero_counter, topic_seed_counter, topic_train_index_list, total_judged, topic_train_percentage_loop_counter, total_document_in_relevance_judgement, topic_estimated_one_counter, topic_estimated_zero_counter, topic_related_documents, topic_non_related_documents, topic_X, topic_y, topic_docIndex_DocNo
...
found_per_budget_per_topic_info = {} # key is the budget count,
found_per_budget_per_topic_prf = {} # key is the budget, value is a list of tuple (topicId, Precison, Recall, F-1 for hybrid judgement)
# value is the list of tuples of topic, r, nr,
# first element is specail budget 0 means seed number of documents

previous_topic_index = -1
current_topic_index = 0
number_of_topic_switches = 0

budget_file_location = base_address + 'budget_protocol:' + protocol + '_batch:' + str(batch_size) + '_seed:' + str(n_labeled) + ".pickle"
print budget_file_location
...
for test_size in test_size_set:
    ...
    for fold in xrange(1,2):
        ...
        topic_related_file = base_address + datasource + "_related_documents_per_topic.pickle"
        topic_non_related_file = base_address + datasource + "_non_related_documents_per_topic.pickle"
        ...
        topic_initial_X_train, topic_initial_Y_train, topic_seed_one_counter, topic_seed_zero_counter, topic_seed_counter, topic_train_index_list, total_judged, topic_loopCounter, total_document_in_relevance_judgement, topic_estimated_one_counter, topic_estimated_zero_counter, topic_related_documents, topic_non_related_documents, topic_X, topic_y, topic_docIndex_DocNo = get_topic_distribution()
        ...
        budget_list.append(datasetsize)
        ...
        topic_distribution = []
        topic_beta_parameters = [] # a list of a and b parameter controlling beta distribution
        topic_oracle_parameters = []

        topic_smart_oracle_parameters = []

        topic_seed_info = [] # list of tuples (topicStringId, seed_relevants, seed_non_relevant)
        topic_seed_info_dictionary = {}

        pickle.dump(topic_related_documents, open(topic_related_file, "wb"))
        pickle.dump(topic_non_related_documents, open(topic_non_related_file,"wb"))

        print topic_non_related_file, len(topic_non_related_documents)



        # each entry is the contrologin for each topic
        # topic_beta_parameters[1] = (2,1) means topic indexed by 1, has two relevant and 1 non relevant documents
        for topic in xrange(start_topic, end_topic):
            #print "Topic:", topic
            if topic in topicSkipList:
                #print "Skipping Topic :", topic
            ...
            print "Initializing topic beta parameter list"

            topic_relevant_documents = BLA_alpha # \alpha
            topic_non_relevant_documents = BLA_beta # \beta

            topic_beta_parameters.append((topic_relevant_documents,topic_non_relevant_documents))
            topic_oracle_parameters.append((topic_related_documents[topic], topic_non_related_documents[topic]))
            topic_smart_oracle_parameters.append((topic_related_documents[topic], topic_non_related_documents[topic]))


            topic_seed_info.append((topic, topic_seed_one_counter[topic], topic_seed_zero_counter[topic]))
            topic_seed_info_dictionary[topic] = (topic_seed_one_counter[topic], topic_seed_zero_counter[topic])

        found_per_budget_per_topic_info[0] = topic_seed_info
        ...
        for budget_limit in budget_list:
            ...
            while total_judged < budget_limit:
                ...
                if deterministic == False:
                    ...
                    # print "Sampling from Topic Beta Parameters"
                    topic_beta_samples_list = [] # index is topic id and content is the sample from beta distribution

                    if MAB == 1 or MAB == 0:
                        if MAB == 1:
                            print "MAB Sampling"
                        elif MAB == 0:
                            print "Round Robin Sampling"
                        for t_index, t_param in enumerate(topic_beta_parameters):
                            # always reinforce the same seed
                            # np.random.seed(len(topic_beta_parameters))
                            if topic_complete_list[t_index] == 0: # means topic still has documents
                                topic_beta_samples_list.append(np.random.beta(t_param[0],t_param[1]))
                                #topic_beta_samples_list.append((float(t_param[0]))/(float(t_param[0]+t_param[1])))

                            else:
                                topic_beta_samples_list.append(0.00) ## forcely putting 0 so that argmax does not pick it

                    elif MAB == 2:
                        print "Oracle Sampling"
                        for t_index, t_param in enumerate(topic_oracle_parameters):
                            # always reinforce the same seed
                            # np.random.seed(len(topic_beta_parameters))
                            if topic_complete_list[t_index] == 0: # means topic still has documents
                                topic_beta_samples_list.append(np.random.beta(t_param[0],t_param[1]))
                        ...
                        print "Smart Oracle Sampling"
                        print topic_smart_oracle_parameters
                        for t_index, t_param in enumerate(topic_smart_oracle_parameters):
                            # always reinforce the same seed
                            # np.random.seed(len(topic_beta_parameters))
                            if topic_complete_list[t_index] == 0: # means topic still has documents
                                topic_beta_samples_list.append(np.random.beta(t_param[0],t_param[1]))
                    ...
                    # if all samples are zero then all topics are exhausted
                    if sum(topic_beta_samples_list) == 0.0 or sum(topic_complete_list) == len(topic_complete_list):
                        print "BREAKING BASED on Beta 0 SAMPLES"
                        break

                    topicIndex = -1
                    if MAB == 1 or MAB == 2 or MAB == 3:
                        # for MAB
                        topicIndex = np.argmax(topic_beta_samples_list)
                    elif MAB == 0: # deteminis
                    # for deteministic
                        topicIndex = deterministic_topic_sample_number % topicUsedListIndex

                    current_topic_index = topicIndex
                    if current_topic_index != previous_topic_index:
                        number_of_topic_switches = number_of_topic_switches + 1
                    previous_topic_index = current_topic_index

                    topic = indexToTopicId[topicIndex]
                ...
                #print "Topic:", topic, "len(train_index_list)", len(train_index_list),
                ...
                if topic_total_document_judged<len(X): # topic still has document
                    if (topic_total_document_judged + 1) < len(X):
                        batch_size = 1
                    else:
                    ...
                    if use_ranker == True:
                        ...
                        if train_per_centage_flag == False:
                            ...
                            print "Updating Topic Beta Parameters"
                            topic_relevant_documents, topic_non_relevant_documents = topic_beta_parameters[topicIndex]
                            topic_relevant_documents = topic_relevant_documents + seed_one_counter
                            topic_non_relevant_documents = topic_non_relevant_documents + seed_zero_counter
                            topic_beta_parameters[topicIndex] = (topic_relevant_documents,topic_non_relevant_documents)
                            # for MAB == 2, we do not update the oracle parameters
                            #print topic_beta_parameters

                            if MAB == 3: # smart oracle
                                topic_rel_doc, topic_non_rel_doc = topic_smart_oracle_parameters[topicIndex]
                                topic_rel_doc = topic_rel_doc - seed_one_counter
                                topic_non_rel_doc = topic_non_rel_doc - seed_zero_counter

                                topic_smart_oracle_parameters[topicIndex] = (topic_rel_doc, topic_non_rel_doc)
                    ...
                    topic_complete_list[topicIndex] = 1
                    ...
                    for t_param in topic_beta_parameters:
                        total_relevant_found = total_relevant_found + t_param[0]

                    found_per_budget[budget_limit] = total_relevant_found
                    print "budget:", budget_limit ,"total found:", total_relevant_found
            ...
            for t_i, t_param in enumerate(topic_beta_parameters):
                topicString = indexToTopicId[t_i]
                # get the seed related document + found in search - 1 for beta paraemter intialization
                r = topic_seed_info_dictionary[topicString][0] + t_param[0] - 1
                nr = topic_seed_info_dictionary[topicString][1] + t_param[1] - 1
                per_budget_detail.append((topicString, r, nr))
                total_relevant_found = total_relevant_found + r
                len_train_list = len(topic_train_index_list[topicString])
                ...
                for train_index in topic_train_index_list[topicString]:
                    doc_label = topic_y[topicString][train_index]
                    docNo = topic_docIndex_DocNo[topicString][train_index]
                    human_label_str = human_label_str + str(topicString) + " " + str(docNo) + " " + str(doc_label) + "\n"
                    # putting these human label for training and predicting in train list
                    train_x.append(topic_X[topicString][train_index])
                    train_y.append(doc_label)

                test_x = []
                test_y = []
                test_DocIndex_DocNo = {}
                # test index 0 to actual DocNo
                test_DocIndex = 0

                for train_index in xrange(0, len(topic_X[topicString])):
                    if train_index not in topic_train_index_list[topicString]:
                        test_x.append(topic_X[topicString][train_index])
                        test_y.append(topic_y[topicString][train_index])
                        test_DocIndex_DocNo[test_DocIndex] = topic_docIndex_DocNo[topicString][train_index]
                        test_DocIndex = test_DocIndex + 1
                ...
                # if all goes to train that means that topic is exhausted manually
                # nothing to predict automatically
                if len(topic_train_index_list[topicString]) != len(topic_X[topicString]):
                    predict_y = lrm.predict(test_x)

                # predict label string start with all human label first


                for test_index in xrange(0, len(test_x)):
                    docNo = test_DocIndex_DocNo[test_index]
                    pred_label_str = pred_label_str + str(topicString) + " " + str(docNo) + " " + str(predict_y[test_index]) + "\n"

                predict_y_all = []
                true_y_all = []

                for y_val in train_y:
                    predict_y_all.append(y_val)
                    true_y_all.append(y_val)
                for y_val in predict_y:
                    predict_y_all.append(y_val)

                for y_val in test_y:
                    true_y_all.append(y_val)

                precision = precision_score(true_y_all, predict_y_all, average='binary')
                recall = recall_score(true_y_all, predict_y_all, average='binary')
                f1score = f1_score(true_y_all, predict_y_all, average='binary')

                per_budget_prf.append((topicString, precision, recall, f1score))

                print "r + nr = ", str(r+nr), "total_judged = " , str(total_judged), "len(train_list)", str(len_train_list)


            human_label_location_final = human_label_location + str(budget_limit) + '_human_qrels.txt'
            text_file = open(human_label_location_final, "w")
            text_file.write(human_label_str)
            text_file.close()

            predicted_location_final = predicted_location_base + str(budget_limit) + '_hybrid_qrels.txt'
            text_file = open(predicted_location_final, "w")
            text_file.write(human_label_str)
            text_file.write(pred_label_str)
            text_file.close()


            found_per_budget[budget_limit] = (total_relevant_found, number_of_topic_switches)
            found_per_budget_per_topic_info[budget_limit] = per_budget_detail
            found_per_budget_per_topic_prf[budget_limit] = per_budget_prf
            ...
            budget_details_location_final = predicted_location_base + str(
                budget_limit_to_train_percentage_mapper[budget_limit]) + '_budget_detail.txt'

            budget_prf_location_final = predicted_location_base + str(
                budget_limit_to_train_percentage_mapper[budget_limit]) + '_budget_prf.txt'

            pickle.dump(found_per_budget, open(budget_file_location, "wb"))
            pickle.dump(learning_curve_budget, open(learning_curve_location, "wb"))
            pickle.dump(precision_curve_budget, open(precision_curve_location, "wb"))
            pickle.dump(recall_curve_budget, open(recall_curve_location, "wb"))
            pickle.dump(found_per_budget_per_topic_info,open(budget_details_location_final,"wb"))
            pickle.dump(found_per_budget_per_topic_prf, open(budget_prf_location_final, "wb"))

            #if total_judged == 2500:
            #    exit(0)


for key in sorted(found_per_budget.iterkeys()):
    print "budget,", key, ",found,", found_per_budget[key]
