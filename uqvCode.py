...
def initialize_query_level_bandits(topicId, topicInfo, topicSortedInfo, topicPoolInfo, topic_qrels, pool_value):
    queryInfo = topicSortedInfo[topicId]
    queryList = list(queryInfo.keys())
...
def query_level_bandits(topicId, topicInfo, topicSortedInfo, topicPoolInfo, topic_qrels, pool_value, topic_bandit_infos_list):
    queryInfo = topicSortedInfo[topicId]
    queryList = list(queryInfo.keys())
...
def topic_level_MTF(topicInfo, topicSortedInfo, topicPoolInfo, topic_qrels, pool_value, data_path, datasource, systemName):
    ...
    for topicId, pooled_budget in sorted(topicPoolInfo.iteritems()):
        ...
        topic_all_infos_bandits[topicId] = copy.deepcopy(initialize_query_level_bandits(topicId, topicInfo, topicSortedInfo, topicPoolInfo, topic_qrels, pool_value))
...
def topic_level_bandit_ns(topicInfo, topicSortedInfo, topicPoolInfo, topic_qrels, pool_value, data_path, datasource, systemName):
    topic_budget = {}
    total_budget = 0
    # topic budget is equal to pooled budget level 10 - 1
    np.random.seed(3)
...
if __name__ == '__main__':
    ...
    topic_budget_docs_MAB_HIL = topic_level_bandit_ns(topicInfo, TopicDiversityInfoSorted, topicPooledBudgetInfo, topic_qrels, pool_value, data_path, datasource, systemName)

    qrel_file_generator_all_HIL(topic_budget_docs_MAB_HIL, "MAB_HIL", data_path, datasource, systemName)
