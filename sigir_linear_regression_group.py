...
for type in plot_type_list:
    ...
    for datasource1 in data_set_list:
        ...
        for source in trainsource:
            ...
            if type == "tau ap":
                raw_data_dictionary = dataset_tausap[source]
        ...
        plt.xticks(system_numbers_list, system_numbers_list, size=3)
        if type == "tau" or type == "tau ap":
        ...
        if type == "maximum drop":
            plt.ylim([0, 20])
            plt.yticks(np.arange(0, 22, step=2), size=8)
        if type == "unique number of documents":
            plt.ylim([1000, 5000])
        ...
        plt.title("Test=" + datasouce_name_toacronym[testsource] + "\n MSE=" + str(mean_squared_error(test_y, y_pred))[:8], size=10)
        plt.tight_layout()
