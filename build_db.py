...
def store_contents(data_path, save_path, preprocess, num_workers=None):
    ...
    with tqdm(total=len(files)) as pbar:
        for pairs in tqdm(workers.imap_unordered(get_contents, files)):
            ...
            # each time only one document in a pair list so pairs[0]
            # and the second elemnet of pairs[0] is the document contents so pairs[0][1]
            # and put those list of words in a another list
            # so [ pairs[0][1]
            dictionary.add_documents([pairs[0][1].split()]) # pairs[0]-->docId, pairs[1]-->documentContent
            pbar.update()
