...
for data_set_name_index, datasource in enumerate(data_set_list):
    ...
    for excluded_systems_index in excluded_systems_index_list:
        ...
        for start_number in group_start_number[datasource]:
            excluded_systems_tau_drop_uniqueDocs_file_name = data_path + datasource + "grp_start_number_" + str(
                start_number) + "_pseudo_qrels_all_taus_drop_unique_counts_group_format_" + str(
                onlyautomatic) + "_" + rankMetric + "_" + str(
                excluded_systems_index_list[0]) + ".pickle"
            ...
            merge_dict(exlcuded_systems_tau_ap_list, excluded_systems_tau_drop_uniqueDocs_object[1])
    ...
    for plot_type in plot_type_list:
        ...
        if plot_type == "tau":
            ...
            if datasource == 'TREC8' or datasource == 'TREC7':
                data_frame, system_numbers_list = update_dataframe_test_run(data_frame, datasource)
            ...
            system_numbers_list = list(sorted(all_drops_for_system_numbers_across_all_shuffles.iterkeys()))
            ...
            system_numbers_list = list(sorted(all_unique_doc_counts_for_system_numbers_across_all_shuffles.iterkeys()))
...
plt.tight_layout()
print plot_address + 'ictir_fig_1'+ rankMetric +'.pdf'
plt.savefig(plot_address + 'ictir_fig_1'+ rankMetric +'.pdf', format='pdf', orientation='landscape')
